...
//EXPORT_SYMBOL_GPL(__nvme_alloc_iod);
...
EXPORT_SYMBOL_GPL(nvme_free_iod); //key_value
...
EXPORT_SYMBOL_GPL(nvme_setup_prps); //key_value
...
int nvme_key_batch_command(struct block_device *bdev, unsigned long arg, int len,int mode) { //only p2p
	...
	if(mode==0){/////sync
		for(i=0;i<len;i++){
			...
			req = blk_mq_alloc_request(ns->queue, WRITE,(GFP_KERNEL), false); //__GFP_WAIT 빼면 어떻게 되나?  | __GFP_WAIT
			if (IS_ERR(req)) {
		...
		for (i = 0; i < len; i++) {
			...
			req = blk_mq_alloc_request(ns->queue, WRITE,
					(GFP_KERNEL), false); //__GFP_WAIT 빼면 어떻게 되나?  | __GFP_WAIT
			if (IS_ERR(req)) {
				printk("request allocation fail!!!!!!\n"); return -1;
			}
			req->timeout = list[i]->timeout;
