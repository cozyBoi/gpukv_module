...
void* pipe_task(void* param){ ///only IO thread
	...
	//	printf("thread %d,%d , status : %d\n",id,io_num,request_status[id][current_chunk]);//!=1) pthread_cond_wait(&cond[id][current_chunk],&cond_lock[id][current_chunk]);
	//	printf("thread %d start!\n",id);
	while(!done){
		if(current_request_start>=request_num){
			pthread_mutex_lock(&io_lock[id]); /*mutex*/
			io_complete[id]++;
			pthread_mutex_unlock(&io_lock[id]); /*mutex*/
			break;
		}
		...
		if(id==0){//GET
			...
			for(j=current_request_start;j<end;j++){
				if(call==0){//no p2p
                    ...
                    //printf("key: %d, value.size(): %d, request_size: %d\n",request_list[j], value.size(), request_size);
                    memcpy(buffer+start, value.data(), request_size);
		...
		//////////chunk complete
		//		printf("DONE thread %d,%d , status : %d\n",id,io_num,request_status[id][current_chunk]);//!=1) pthread_cond_wait(&cond[id][current_chunk],&cond_lock[id][current_chunk]);
		...
		if(io_complete[id]<IO_NUM){
			...
			//			printf("END thread %d,%d ,current_chunk %d,  status : %d\n",id,io_num,current_chunk, request_status[id][current_chunk]);//!=1) pthread_cond_wait(&cond[id][current_chunk],&cond_lock[id][current_chunk]);
			pthread_mutex_unlock(&cond_lock[id][current_chunk]);
		}
...
void* chunk_task(void* param){ ///only IO thread
	...
	while(!done){
		...
		//		printf("chunk status %d\n",request_status[0][current_chunk]);
		...
		CUDA_SAFE_CALL(cudaMemcpyAsync(cuda_memory+start,get_pin+start,transfer_size,cudaMemcpyHostToDevice,stream[current_chunk]));
		CUDA_SAFE_CALL(cudaEventRecord(event[0][current_chunk],stream[current_chunk]));
		CUDA_SAFE_CALL(cudaEventSynchronize(event[0][current_chunk]));
		////here
		if(request_mode==0){
			...
			/////////////KERNEL
			if(request_mode==2){
				test_modify<<<cuda_block,nthreads,0,stream[current_chunk]>>>(cuda_memory+start,cuda_put+out_start,remain,request_size);
			}
			else if(request_mode==3){
				test_app<<<cuda_block,nthreads,0,stream[current_chunk]>>>(cuda_memory+start,cuda_put+out_start,remain,request_size);
			}
			else if(request_mode==4){
				test_modify2<<<cuda_block,nthreads,0,stream[current_chunk]>>>(cuda_memory+start,cuda_put+out_start,cuda_reset,cuda_cnt, remain,request_size);
				cuda_cnt++;
			}
			CUDA_SAFE_CALL(cudaEventRecord(event[1][current_chunk],stream[current_chunk]));
			CUDA_SAFE_CALL(cudaEventSynchronize(event[1][current_chunk]));

			////////////////////////////////////////////////////////////
			///////////PUT
			CUDA_SAFE_CALL(cudaMemcpyAsync(put_pin+out_start,cuda_put+out_start,out_transfer_size,cudaMemcpyDeviceToHost,stream[current_chunk]));
			CUDA_SAFE_CALL(cudaEventRecord(event[2][current_chunk],stream[current_chunk]));
			CUDA_SAFE_CALL(cudaEventSynchronize(event[2][current_chunk]));
			///////////////////////////////////////////////////////////
...
int main(int argc, char** argv)
{
	...
	while(1){
		...
		switch(c){
			...
			case 'q':
				...
				break;
			case 'h':
				printf("syntax: %s -s <request_size> -n <num_request> -m <mode:get-0,put-1,get_and_put-2,program-3> -p <num_polling_thread> -i <process:one-0,batch_async-1,batch_sync-2> -q <batch_size> -b <gpu block> -t < gpu theads in block> -c <call:0-cudamemcpy/1-p2p> -h\n",argv[0]);
				exit(1);
				break;
			default:
	...
	char *user_put,*user_get,*get_pin,*put_pin;
    ...
    //////////// rocksdb
    DB *db;
    const char DBPath[] = "/home/asknor/nvme/rocks_prim";
    Options options;
	...
	printf("ORIGINAL | size : %d KB, num : %d, mode : %d, user_thread : %d, block : %d, thread : %d, p2p_mode : %d\n",	request_size/1024, request_num,request_mode, num_thread, gpu_block,gpu_thread,call);
	printf("ORIGINAL | user_thread : %d, chunk_size : %lf MB, chunk_request_cnt : %d, next_key %d\n",num_thread, (double)chunk_size/(1024*1024),chunk_cnt,next_key);
	//	return 0;
	stream=(cudaStream_t*)malloc(sizeof(cudaStream_t)*num_thread);
	event[0]=(cudaEvent_t*)malloc(sizeof(cudaEvent_t)*num_thread);
	event[1]=(cudaEvent_t*)malloc(sizeof(cudaEvent_t)*num_thread);
	event[2]=(cudaEvent_t*)malloc(sizeof(cudaEvent_t)*num_thread);

	for(i=0;i<chunk_num;i++){
		CUDA_SAFE_CALL(cudaStreamCreate(&stream[i]));
		CUDA_SAFE_CALL(cudaEventCreateWithFlags(&event[0][i],cudaEventBlockingSync|cudaEventDisableTiming));
		CUDA_SAFE_CALL(cudaEventCreateWithFlags(&event[1][i],cudaEventBlockingSync|cudaEventDisableTiming));
		CUDA_SAFE_CALL(cudaEventCreateWithFlags(&event[2][i],cudaEventBlockingSync|cudaEventDisableTiming));
	...
	printf("ORIGINAL | Total time: %lf s\tSize: %lf MB\tBandwidth %lf GB/s \n\n\n", total_time,(total_size/(1<<20)),(total_size/(1<<30))/total_time  );///9));
	...
	for(i=0;i<num_thread;i++){
		CUDA_SAFE_CALL(cudaEventDestroy(event[0][i]));
		CUDA_SAFE_CALL(cudaEventDestroy(event[1][i]));
		CUDA_SAFE_CALL(cudaEventDestroy(event[2][i]));
		CUDA_SAFE_CALL(cudaStreamDestroy(stream[i]));
	}
	free(stream); free(event[0]); free(event[1]); free(event[2]);
