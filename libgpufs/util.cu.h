...
#define BEGIN_BLOCK_SINGLE_THREAD if(threadIdx.x+threadIdx.y+threadIdx.z ==0) {
#define END_BLOCK_SINGLE_THREAD  } __syncthreads();
//////////////////////
...
#define NUM_THREADS (blockDim.x * blockDim.y * blockDim.z)
#define BLOCK_ID (blockIdx.x + gridDim.x * blockIdx.y + gridDim.x * gridDim.y * blockIdx.z)

////////////////////////////////////addddddddddddddddd 계속 추가하겠네 --
#define GLOBAL_TID (BLOCK_ID * NUM_THREADS + TID)
#define GLOBAL_WARP_ID (BLOCK_ID*NUM_WARPS + WARP_ID )
#define NUM_BLOCK (gridDim.x*gridDim.y*gridDim.z)
#define GPU_PMEM (128*1024*1024)
#define PAGE_SIZE 4096 // 적어도 align 4KB는 맞춰줘야됨!
...
/////KV
__forceinline__ __device__ unsigned long readNoCache(const volatile unsigned long* ptr) {
	unsigned long v = *ptr;
	return v;
}

__forceinline__ __device__ char2 readNoCache(const volatile unsigned char* ptr) {
...
////KV need to be change shfl_sync syntax
__device__ inline BroadcastHelper broadcast(BroadcastHelper b, int leader = 0)
{
	BroadcastHelper t;

	t.i[0] = __shfl_sync(WARP_MASK, b.i[0], leader );
	t.i[1] = __shfl_sync(WARP_MASK, b.i[1], leader );
...
__device__ inline void graph_parse_vertex(int src, char *data, int *column_indices,
		int offset, int &tot, int stride=128){
	int tid = blockIdx.x * blockDim.x + threadIdx.x;
	int idx = 0;
	tot = 0;
	tot |= data[offset];
	tot |= data[offset+1] << 8;
	tot |= data[offset+2] << 16;
	tot |= data[offset+3] << 24;
	idx += 4;

	for(int i = 0; i < tot; ++i){
		column_indices[tid*128+i] = 0;
		column_indices[tid*128+i] |= (unsigned char)data[offset+idx];
		column_indices[tid*128+i] |= (unsigned char)data[offset+idx+1] << 8;
		column_indices[tid*128+i] |= (unsigned char)data[offset+idx+2] << 16;
		column_indices[tid*128+i] |= (unsigned char)data[offset+idx+3] << 24;
		// ignore weight
		idx += 8; 
	}
}

__device__ inline void graph_parse_all(int src, char *data, int *column_indices,
		float *weights, int offset, int &tot, int stride=128){
	int tid = blockIdx.x * blockDim.x + threadIdx.x;
	int idx = 0;
	unsigned int tmp;
	tot = 0;
	...
	for(int i = 0; i < tot; ++i){
		...
		// weight
		tmp = 0;
		tmp |= (unsigned char)data[offset+idx+4];
		tmp |= (unsigned char)data[offset+idx+5] << 8;
		tmp |= (unsigned char)data[offset+idx+6] << 16;
		tmp |= (unsigned char)data[offset+idx+7] << 24;
		weights[tid*128+i] = *(float*)&tmp;
		idx += 8;
	}
}
